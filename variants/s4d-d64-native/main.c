...
#define PATCH_SIZE   1       /* 4x4 patches */
#define GRID_SIZE    64      /* IMG_SIZE / PATCH_SIZE */
#define SEQ_LEN      4096     /* GRID_SIZE*GRID_SIZE patches after the Hilbert scan */
#define IN_CHANNELS  3       /* RGB */
#define PATCH_DIM    3      /* IN_CHANNELS * PATCH_SIZE * PATCH_SIZE */
#define D_MODEL      64     /* hidden dimension */
#define D_STATE      64     /* S4D state dim (54 complex pairs) */
#define HALF_STATE   32      /* D_STATE / 2 */
#define N_LAYERS     2       /* number of stacked S4D layers */
#define N_CLASSES    4       /* galaxy classes */
/* 256(idx) + (108*48+108) + 3*(108 + 108*54 + 108*54 + 108*54*2 + 108) + (4*108+4) */
#define WEIGHTS_SIZE_FLOATS 21252
...
void model_forward(
    ...
    take_last_timestamp(bufA, pooled);                                                     TICK(6);
